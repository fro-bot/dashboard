...
**Verification:**
- The artifact states, for every fact the page will publish, which upstream owns the behavior. It records no tag and no verification date: a hand-maintained version string rots silently, and the claims are re-surveyed when Gateway push behavior changes rather than tracked against a pin.
...
- **Request path — the layer this plan originally missed.** A public request is `browser → Caddy → Hono → (service worker, on repeat visits)`. This plan modelled only the last two, and the omission is what broke the release: Caddy rewrites every extensionless path to `/` before proxying, so `GET /privacy` reached the app as `GET /` and was correctly redirected to login. That config lives in `marcusrbrown/infra`, so no change or test in this repository could surface it. A public extensionless route needs a change at all three layers. Paths *with* a file extension bypass the rewrite, which is why the gap stayed invisible until a clean URL needed it. Written up in `docs/solutions/integration-issues/public-route-swallowed-by-caddy-extensionless-rewrite-2026-09-20.md`.
- **Error propagation:** The page has no runtime data dependency and must never depend on a remote read — a compliance document that can fail to render is worse than one that is slightly stale. Its *delivery* is a different matter and has several failure modes: build output, static serving, edge routing, and service-worker navigation. Absence of a data path is not absence of failure modes, and treating the two as the same is what produced the production failure recorded below.
